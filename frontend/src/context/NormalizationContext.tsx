...
import {
  MappingRow, PermissionRequirement, ConfidenceExplanation,
...
interface NormalizationApiResponse {
  runId: string;
  rows: MappingRow[];
...
async function fetchNormalization(runId: string): Promise<MappingRow[]> {
  const data = await apiGet<NormalizationApiResponse>(
    `/api/runs/${runId}/normalization`,
  );
  return data.rows ?? [];
}
...
export function NormalizationProvider({ children }: { children: React.ReactNode }) {
  ...
  // Rows — fetched from live endpoint
  ...
  useEffect(() => {
    ...
    fetchNormalization(runId)
      ...
      .catch((e) => {
        if (!alive) return;
        setRowsError(e?.message ?? 'Failed to load normalization data');
      })
      .finally(() => {
  ...
  const [permFetchCount, setPermFetchCount] = useState(0);
  ...
  useEffect(() => {
    ...
    fetchPermissions()
      .then((data) => { if (alive) { setPermissions(data); setPermissionsError(null); } })
      .catch((e) => { if (alive) setPermissionsError(e?.message ?? 'Failed to load permissions'); })
      .finally(() => alive && setPermissionsLoading(false));
    return () => { alive = false; };
  }, [permFetchCount]);

  // Filter / sort / selection state — unchanged from original
  ...
  const counts = useMemo(() => {
    ...
    for (const r of filtered) {
      if (r.status in c) c[r.status as Tab]++;
    }
    return c;
  ...
  const relevantPermissions = useMemo(
    () => selectedRow ? permissions.filter(p => p.sourceSystem === selectedRow.sourceSystem) : [],
  ...
  const value = useMemo(() => ({
    ...
    rows, rowsLoading, rowsError,
    permissions, confidence, permissionsLoading, permissionsError, refetchPermissions,
