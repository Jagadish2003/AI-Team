...
function DecisionBadge({ value }: { value: string }) {
  ...
  const label = value === 'APPROVED' || value === 'REJECTED' ? value : 'PENDING';
  return (
    <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold tracking-wide whitespace-nowrap ${cls}`}>
    {label}
    </span>
...
export default function OpportunityList({
  ...
  const dropdownBtn =
  'flex items-center gap-2 px-3 py-1.5 rounded-lg border border-border bg-panel2 text-xs text-text hover:border-accent/50 hover:text-accent transition-colors cursor-pointer w-full';
    const dropdownPanel =
    ...
    `px-3 py-2 text-xs cursor-pointer transition-colors ${
      active ? 'bg-accent font-medium text-white' : 'text-text hover:bg-accent/15 hover:text-accent'
    }`;
    ...
    return (
      ...
      <h2 className="text-lg font-semibold text-text">Opportunities</h2>
      ...
      className="w-full rounded-md border border-border bg-bg/50 px-3 py-2 pr-10 text-sm text-text placeholder:text-muted hover:border-accent/50 transition-colors focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/50"
      />
