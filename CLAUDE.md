...
* Native DB connector credentials (Oracle/PostgreSQL/SQL Server) resolve per org from the encrypted vault — R17-D3 Addendum A T11 wired the pool to the single credential path: `connectors/db/connection_pool.py` `resolve_db_credentials(config)` calls `get_connector_credentials(config.org_id, config.connector_id)` first (a static credential entered per org via the Integration Hub form under connector id `oracle_db`/`postgresql`/`sql_server`), and only falls back to the env `resolve_secret(username_key/password_key)` for CLI/standalone. The service-account USERNAME/PASSWORD env vars were purged from `.env`/`.env.example` (AC12); use the Integration Hub form or `scripts/migrate_env_credentials_to_vault.py` to vault them.
* Oracle native DB connector env vars (instance config, non-secret): `ORACLE_HOST`, `ORACLE_PORT` (default `1521`), `ORACLE_DATABASE` (service name, default `ORCL`). The `ORACLE_DB_USERNAME`/`ORACLE_DB_PASSWORD` pair is only a CLI/standalone fallback (`username_key`/`password_key`) — the credential of record lives in the vault, not the env templates.
* PostgreSQL native DB connector env vars (instance config, non-secret): `POSTGRESQL_HOST`, `POSTGRESQL_PORT` (default `5432`), `POSTGRESQL_DATABASE` (default `postgres`), and `POSTGRESQL_SSL_MODE` (`require`/`prefer`/`disable`). The `POSTGRESQL_USERNAME`/`POSTGRESQL_PASSWORD` pair is only a CLI/standalone fallback — the credential of record lives in the vault. Same rationale as Oracle above.
