...
* `backend/discovery/ingest/slack.py`: Slack change-based ingestor (`connector_id='slack'`) with TWO paths off ONE `(org, 'slack')` checkpoint — no new connector, no new checkpointing. **Reach (R16-A2)**: reads structured message SIGNAL (activity/escalation/cross-reference markers via `slack_signals.py`) with a per-channel `ts` cursor map; public-member-live channels only, narrowed to the P5 selection (`org_connector_get(org,'slack').channels`). **Depth (R18-A4 / AT-594, T1)**: `ingest_deep_content(org_id, records)` runs BESIDE the unchanged signal path — assembles the same delta records into threads (grouped by `thread_ts`/parent `ts`) or time-bounded windows (`THREAD_WINDOW_SECONDS`, for messages with no thread structure), re-checks scope against the P5 selection (`_selected_scope_channel_ids` — private/DM/non-member/archived/unselected excluded, AC2), renders author-attributed text, and hands each thread to `retrieval.ingest_content` as a `ContentArtifact` (`source_system='slack'`, `content_type='conversation'`, thread-level `origin='observed'` evidence pointer at `"{channel_id}:{thread_key}"`, AC1). Never writes vectors. The runner drives both paths in ONE `change_runner` pass (`runner.py` `_ingest_slack_corroboration`); the deep hand-off is non-blocking there (reach + depth share the checkpoint, so a substrate failure is logged, not fatal — idempotent replace re-indexes on next change). Slack/Teams conversation evidence stays capped at MEDIUM (T2 does the Teams counterpart; T3 adds edit/delete freshness — out of AT-594 scope).
