...
* `backend/discovery/ingest/azure_events.py` (+ `azure_events_config.py`, `azure_alerts.py`): MSP-B2 Azure Event Connector — the Azure half of the MSP-B1/B2 pair. T1 (AT-648) AUTHENTICATION + subscription-access foundation; T2 (AT-649) Azure Monitor **Alerts** polling (Activity Log + Service Health remain T3). `AzureEventIngestor` is a `ChangeBasedIngestor` (reuses the existing change-based abstraction, same as the B8 bridge — migrates cleanly to the B1 skeleton). `ingest_alerts` polls each PINNED subscription independently with its OWN checkpoint (opaque per-subscription `{subscription_id: last_firedDateTime}` map via `encode/decode_checkpoints`), keeps only alerts strictly newer than that checkpoint (`azure_alerts.filter_new_alerts` — no re-reads, T2-AC2/AC4), normalises each through the B0 `map_azure_monitor` (T2-AC3), and emits a transport record carrying the subscription as `account_scope` on the WRAPPER (never a connector-invented detector field). A subscription's checkpoint advances ONLY after its alerts process cleanly; a per-subscription fetch failure is caught, reported loudly in `subscription_status`, and leaves that subscription's checkpoint unadvanced while the others continue (no silent thinning); a single malformed alert is loud-skipped without failing the subscription. `azure_alerts.py` owns the injectable client (`FixtureAzureAlertsClient` offline / `HttpAzureAlertsClient` live outbound-only GET — Alerts Management ONLY, scope defence). Auth REUSES the existing Azure AD plumbing — `AzureEventIngestor.acquire_token` resolves a per-org service principal from the vault (`app.auth.vault` static-credential machinery: username=client_id, secret=client_secret, base_url=tenant_id — no new vault path) and mints an ARM-scoped token via the SHARED client-credentials exchange `app.auth.oauth.request_client_credentials_token` (extracted from `get_client_credentials_token` so there is ONE token-exchange implementation, not a second copy for the vault path). Environment-aware from day one via the SINGLE shared map `app/azure_environments.py` (MSP-B2 T4 / AT-651 — `azure_events_config` re-exports it, no duplicate endpoint map): `AzureCloud` → login.microsoftonline.com / management.azure.com, `AzureUSGovernment` → login.microsoftonline.us / management.usgovcloudapi.net; `resolve_environment(name)` is config-driven (default AzureCloud, unknown → error), ARM scope is `{resource_manager}/.default`. That shared module is dependency-free/credential-free and is the one Azure environment source any Azure integration reuses (this connector now; the model gateway's Azure OpenAI Government surface per B9). Both access modes work — Lighthouse (one SP/home tenant, many delegated subscriptions) and direct (per-tenant SP); the token is minted against the SP's home tenant either way. SUBSCRIPTION DISCIPLINE (AC4/AC7): the pinned `subscriptions` set in config is the ONLY set ingested — `authorized_subscriptions()` never includes an unpinned delegated subscription, `newly_delegated()`/`pending_delegated_subscriptions()` report delegated-but-unpinned candidates for Owner approval without ingesting them, so the connected estate never auto-grows. Config is non-secret (offline `fixtures/azure_events_config_sample.json`; live `AZURE_EVENT_CONFIG` JSON, org-keyed or flat); an inline secret is rejected via the shared `operational_config.find_inline_secret_keys` guard. The connector is transport-only (no detector fields) and does no module-level `app` import (offline-safe).
