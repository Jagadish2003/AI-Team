...
* Git content (`git_content`, R18-A2) ingestion is likewise configured, not discovered: the repositories AgentIQ may read content from are declared per deployment in `GIT_CONTENT_REPOS` (a JSON array of secret-free repo configs — `repo_id`, `branch`, local-clone `path`; the offline fixture `git_content_sample.json` is used when unset). Content is opt-in per repository. Distinct from the GitHub *signal* connector's OAuth flow — the content reader shells out to `git` in the declared local clone.
