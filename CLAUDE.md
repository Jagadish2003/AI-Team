...
* `backend/discovery/enterprise_apps/structure.py`: R18-A6 / AT-606 (T1) phase-two application **structure** extraction over repository content already ingested by R18-A2 — the code-and-structure counterpart to the R17-A3/A4 operational (runtime-signal) phase. Pure (no DB, no `app` import), DETERMINISTIC, and NO LLM in the extraction path (structure is observed, never inferred — a structural test pins the no-model-import rule). `extract_structure(repo_content, platform)` emits the platform-agnostic shared model `AppStructure` = `Component` (Spring stereotypes → `service`/`controller`/`repository`/`configuration`/`component`, plus Maven/Gradle `module`s) + `Dependency` (build-declared, versioned — Maven `pom.xml` with `${property}`/`project.version` resolution, Gradle string/map/`platform(...)` notation) + `Endpoint` (declared REST surface — class-level `@RequestMapping` base joined to method `@GetMapping`/`@PostMapping`/… and `@RequestMapping(method=…)`) + `config_shape` (Spring `application*.yml|.yaml|.properties` / `bootstrap.*` — KEYS kept, every VALUE replaced with `REDACTED`; AC6). Platform dispatch registry `_PARSERS`: only `java` is implemented in T1; `dotnet` raises `NotImplementedError` (the T2 seam), an unknown platform raises `ValueError`. Rides A2's content mover — no second code fetch/redaction path.
