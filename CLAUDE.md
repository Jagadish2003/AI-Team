...
* `backend/discovery/ingest/teams.py`: R17-A1 Microsoft Teams change-based ingestor. Implements `ChangeBasedIngestor` over channel messages via the Microsoft Graph delta query; reuses the Graph OAuth app registration (see `configs.py`). Opaque checkpoint is a per-channel map keyed by `"{team_id}/{channel_id}"` — a Graph `@odata.deltaLink` when live, an ISO `lastModifiedDateTime` high-water marker when running offline fixtures (never a fixture-only synthetic `change_seq`). Live: `TeamsGraphClient` is a context manager, retries Graph `429` throttling with `Retry-After` (bounded), and is closed after every ingest; the per-channel deltaLink advances only on that channel's last page. `reports_deletes=False` (Graph `@removed` items are deferred, paired with the SharePoint deletion deferral).
* `backend/discovery/ingest/teams_signals.py`: R17-A1 Teams operational signal extraction — cross-reference marker detection, conversation activity, and escalation signals, plus `build_teams_corroboration_payload`. Teams is a conversation source subject to the MEDIUM corroboration ceiling (`t3_ceiling_clamp.py`, `CONVERSATION_SOURCE_IDS`). `extract_cross_reference_markers` strips HTML from Graph message bodies (they arrive as HTML) before reusing the Slack marker extractor.
* `backend/discovery/ingest/sharepoint.py`: R17-A2 / AT-459 (T2) SharePoint change-based ingestor. Implements `ChangeBasedIngestor` over document libraries via the Microsoft Graph drive delta query (`/drives/{id}/root/delta`); reuses the Teams (R17-A1) Graph auth pattern. Opaque checkpoint is a per-library delta-token map keyed by `"{site_id}/{drive_id}"` (high-water `lastModifiedDateTime` marker, mirroring the Teams connector). Reach phase: reads document metadata SIGNAL + observed `EvidencePointer` only — no document-body content (that is 1.8). `reports_deletes=False` (Graph `deleted`-facet items are skipped, matching the paired Teams `@removed` deferral). Signal aggregation (T3), OAuth connect wiring (T5), and runner integration are separate subtasks — not wired into the live run yet.
...
* OAuth client secrets (production, enforced by `REQUIRE_CONNECTOR_SECRETS`): `SALESFORCE_CLIENT_SECRET`, `SERVICENOW_CLIENT_SECRET`, `JIRA_CLIENT_SECRET`, `CONFLUENCE_CLIENT_SECRET`, `GITHUB_CLIENT_SECRET`, `SLACK_CLIENT_SECRET`, `TEAMS_CLIENT_SECRET`, `SHAREPOINT_CLIENT_SECRET`, `SAP_CLIENT_SECRET`, `DYNAMIC365_CLIENT_SECRET`. Teams (Microsoft Graph) also reads `TEAMS_TENANT_ID` (Entra tenant GUID, or `organizations`; default `organizations`) — see `app/auth/configs.py`. SharePoint (R17-A2 / AT-462) reuses the Teams Microsoft Graph app registration: `SHAREPOINT_CLIENT_ID` defaults to `TEAMS_CLIENT_ID` and `SHAREPOINT_TENANT_ID` defaults to `TEAMS_TENANT_ID`, so one Graph app can serve both connectors; set them only to point SharePoint at a dedicated app/tenant. Its client secret still follows the per-connector convention (`SHAREPOINT_CLIENT_SECRET`) — set it to the shared app's secret when reusing the Teams app. SharePoint's scopes are the minimal read-only `Sites.Read.All` + `offline_access` (no write); Confluence's are the read-only `read:confluence-content.all` + `read:confluence-space.summary` + `offline_access`.
