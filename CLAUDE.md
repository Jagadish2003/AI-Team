...
* `backend/discovery/enterprise_apps/component_retrieval.py`: R18-A6 / AT-610 (T5) **component-scoped retrieval** — bridges the structure map into the platform retrieval API (`app.retrieval.api.retrieve`) so a query can be scoped to one component's REAL files, never a path/name coincidence (AC5). `resolve_component_artifacts(org_id, app_id, component_ref, content_provider)` resolves the app's configured repo scope, extracts structure PER REPO (not `app_repo_map.extract_app_structure`'s flattened union, so each match can be paired with the repo it came from), and returns the EXACT `"{repo_id}:{path}"` ids — byte-for-byte `discovery.ingest.git_content`'s own `source_artifact` convention — of every file where a component matching `component_ref` (exact match on `qualified_name` or simple `name`, never a substring) was declared. `retrieve_component_code(...)` calls `retrieve(..., artifact_filter=...)` when a `query_text` is given (a normal semantic search scoped to the component), or lists the component's currently-indexed chunks directly (`store.list_chunks_by_artifacts`, similarity `1.0`) when no query is given. Raises `EnterpriseAppConfigError` only when `app_id` itself is unconfigured; an unmatched component or un-indexed content returns `[]` — a scoping miss, not an error.
