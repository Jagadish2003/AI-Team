...
* `backend/discovery/ingest/dotnet_app.py`: R17-A4 .NET application change-based ingestor — the .NET counterpart to `java_app.py`. Reads the OPERATIONAL surface only (ASP.NET Core health checks + EventCounters via `DotNetAppClient`, the `diagnostics_url` endpoint field, .NET log formats) — never source code or external APM. Opaque per-app `{log_offset, metrics_ts}` checkpoint. Produces the observed, provenance-stamped records the corroboration signal is built from.
* `backend/discovery/ingest/dotnet_app_config.py`: R17-A4 per-deployment .NET app target configuration + vault credential resolution. Targets come from config (`DOTNET_APP_TARGETS` / offline fixture), never network scanning; declares a `diagnostics_url` (vs Java's `actuator_url`). Inline secrets in config are rejected and credentials resolve from the vault, never logged.
* `backend/discovery/ingest/dotnet_app_signals.py`: R17-A4 .NET operational signal → corroboration shaping. Stamps every record with an OBSERVED `EvidencePointer` (`source_system='dotnet_app'`, `origin='observed'`) and builds the COR-10 corroboration payload under the `dotnet_app` key. It REUSES the Java friction interpretation (`java_app_signals.build_java_app_signal`, whose output is platform-agnostic) via a small field/level adapter — no duplicated extraction, no separate .NET confidence model.
...
* `docs/INTEGRATE_*.md`: integration guides per connector — includes `docs/INTEGRATE_JAVA_APP.md` (R17-A3 Java application ingestion) and `docs/INTEGRATE_DOTNET_APP.md` (R17-A4 .NET application ingestion → corroboration, COR-10).
...
* Operational-app friction is first-class OBSERVED evidence and plugs into the SAME cross-system corroboration engine (no separate confidence model): **COR-09** (Java, keyed `java_app`) and **COR-10** (.NET, keyed `dotnet_app`) are *elevating* corroborators (MEDIUM→HIGH like ServiceNow/Jira), not Slack-capped. They are gated on the source being connected and only fire when its `operational_friction` block is fresh; a lone operational source stays MEDIUM (single-source, COR-08). Adding a new corroboration rule = registry entry (`corroboration_rules.py`) + engine check + `_CORROBORATION_RULE_SYSTEMS` mapping + `_build_label` priority + a contract test.
