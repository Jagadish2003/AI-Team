...
def check_env() -> tuple:
    ...
    # Derive from URL if not explicitly set (mirrors start.sh logic)
    if not pg_user:
...
# ---------------------------------------------------------------------------
# Credential helpers (same pattern as push-ecr.py)
# ---------------------------------------------------------------------------
...
def pull_images(access_key: str, secret_key: str) -> None:
    ...
    try:
        ...
        # Write token to Docker config (no network call — same fix as push-ecr.py)
...
# ---------------------------------------------------------------------------
# Compose env generation (mirrors start.sh section 3)
# ---------------------------------------------------------------------------
...
def generate_compose_env(pg_user: str, pg_pass: str, pg_db: str,
    ...
    # Escape lone $ in .env values so docker compose doesn't expand them
...
def main() -> None:
    ...
    # Clear AWS credentials from memory now — no longer needed
