...
reporter in isolation (no DB, no live HTTP).
"""
...
def test_offline_loads_targets_from_fixture():
    ...
    assert {"orders-api", "billing-svc"} <= ids
    orders = next(t for t in targets if t.app_id == "orders-api")
    assert orders.diagnostics_url and orders.log_source
    assert orders.environment == "production"        # environment info captured
    assert orders.service == "orders"                # from metadata, used for linkage
    assert orders.credential_ref == DEFAULT_CREDENTIAL_REF
...
def test_live_reads_targets_from_env_json(monkeypatch):
    ...
    monkeypatch.setenv(
        "DOTNET_APP_TARGETS",
        json.dumps([
            {"app_id": "cart", "name": "Cart", "diagnostics_url": "https://c/healthz",
             "log_source": "https://c/logs", "environment": "staging",
             "metadata": {"service": "cart"}}
        ]),
    )
    targets = load_targets("org-9")
    assert [t.app_id for t in targets] == ["cart"]
...
def test_target_needs_at_least_one_operational_surface():
...
@pytest.mark.parametrize(
    "secret_field",
    ["token", "password", "username", "api_key", "connection_string", "certificate", "pfx"],
)
def test_inline_secret_in_target_is_rejected(monkeypatch, caplog, secret_field):
    ...
    monkeypatch.setenv(
        "DOTNET_APP_TARGETS",
        json.dumps([
            {"app_id": "leaky", "diagnostics_url": "https://x/healthz",
...
def test_target_has_no_field_that_could_hold_a_plaintext_secret():
    ...
    for attr in ("token", "password", "username", "secret", "api_key",
                 "connection_string", "certificate"):
...
def test_secret_custom_credential_ref_uses_namespaced_env():
    secret = resolve_secret(
        "org-1", _target(ref="orders_diag"),
        connector_lookup=lambda ref: None,
        env={"ORDERS_DIAG_TOKEN": "NS-TOKEN"},
    )
