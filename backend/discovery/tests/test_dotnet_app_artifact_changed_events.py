...
_ORDERS = [
    "orders-api:diagnostics:2026-07-01T08:00:00+00:00",
    "orders-api:diagnostics:2026-07-01T08:05:00+00:00",
    "orders-api:log:1",
    "orders-api:log:2",
]
_BILLING = [
    "billing-svc:diagnostics:2026-07-01T08:00:00+00:00",
    "billing-svc:log:1",
]
_ALL = _ORDERS + _BILLING
...
def test_events_cover_both_operational_surfaces(captured):
    ...
    assert any(":diagnostics:" in a for a in ids)  # health/diagnostics samples
...
def test_incremental_emits_only_newly_changed_artifacts(captured):
    # Checkpoint mid orders-api (after log 1 / first sample); billing absent.
    store = Store()
    store.save(
        Checkpoint.create(
            "dotnet_app", "org-1",
            _encode_checkpoint(
                {"orders-api": {"log_offset": 1, "sample_ts": "2026-07-01T08:00:00+00:00"}}
            ),
        )
    )
    ...
    assert "orders-api:diagnostics:2026-07-01T08:00:00+00:00" not in emitted  # equal ts
...
def test_runner_helper_drives_dotnet_ingestor_through_shared_runner(monkeypatch):
    """``runner._ingest_dotnet_app_changes`` must drive the real DotNetAppIngestor
    ...
    result = runner._ingest_dotnet_app_changes("org-1", "run-1")
