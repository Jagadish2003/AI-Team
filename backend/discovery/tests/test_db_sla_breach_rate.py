...
Covers:
  ...
  - does not fire when signal is marked degraded
  - SIGNAL_METRICS defines the three required metrics
  - raw_evidence includes all required fields
"""
...
class TestDbSlaBreachRateDetector:
    ...
    def test_does_not_fire_when_volume_guard_fails(self):
        from discovery.detectors.db_sla_breach_rate import detect
        # breach_rate is high but total_tickets_30d < 10
    ...
    def test_does_not_fire_when_sla_breach_key_absent(self):
        from discovery.detectors.db_sla_breach_rate import detect
        results = detect({"other_signal": {"foo": 1}})
        assert results == []

    def test_does_not_fire_when_degraded(self):
        from discovery.detectors.db_sla_breach_rate import detect
        results = detect(_make_signal(breach_rate_pct=99.0, total_tickets_30d=500, degraded=True))
        assert results == []
    ...
    def test_evaluate_returns_unfired_for_degraded_signal(self):
        from discovery.detectors.db_sla_breach_rate import evaluate
        ev = evaluate(_make_signal(breach_rate_pct=99.0, total_tickets_30d=500, degraded=True))
        assert ev.fired is False
        assert ev.raw_evidence["degraded"] is True
