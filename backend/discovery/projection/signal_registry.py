...
_PROFILES: Dict[str, DetectorSignalProfile] = {
    ...
    # ---- Financial Services Cloud (2.0-D1) -------------------------------
    # Field names copied from each fsc_*.py detector's own SIGNAL_METRICS list.
    # The BFSI guardrail applies to every residual here: a credit, suitability,
    # or compliance decision is never presented as something an agent takes.
    "FSC_SERVICING_REQUEST_RECURRENCE": DetectorSignalProfile(
        concept=CONCEPT_RECURRENCE,
        movement_signal="recurrence_count",
        volume_signal="distinct_financial_accounts",
        instance_signal="recurrence_count",
        manual_step="re-handling the same servicing request for a household by hand",
        unit="count",
        case_noun="repeat servicing requests per household",
        residual="requests that need an adviser's judgement on the client relationship",
    ),
    "FSC_REFERRAL_HANDOFF_FRICTION": DetectorSignalProfile(
        concept=CONCEPT_REASSIGNMENT,
        movement_signal="avg_hops",
        volume_signal="referral_count",
        instance_signal="total_hops",
        manual_step="manually re-routing a referral between teams to find the right owner",
        unit="count",
        case_noun="referral handoffs between teams",
        residual="referrals whose correct owner is genuinely ambiguous",
    ),
    "FSC_APPROVAL_REVIEW_CYCLE": DetectorSignalProfile(
        concept=CONCEPT_AGEING,
        movement_signal="median_dwell_days",
        volume_signal="pending_count",
        instance_signal="pending_count",
        manual_step="chasing reviewers by hand to move a pending review forward",
        unit="days",
        case_noun="pending reviews waiting on a chase",
        residual="the suitability and compliance decision itself, which stays with a reviewer",
    ),
    "FSC_CROSS_OBJECT_REWORK": DetectorSignalProfile(
        concept=CONCEPT_RECURRENCE,
        movement_signal="rework_touches",
        volume_signal="records_considered",
        instance_signal="rework_touches",
        manual_step="re-entering the same fact on several records and reconciling them by hand",
        unit="count",
        case_noun="records maintaining the same fact more than once",
        residual="records whose versions of a fact genuinely disagree",
    ),
    "FSC_SERVICE_QUEUE_AGEING": DetectorSignalProfile(
        concept=CONCEPT_AGEING,
        movement_signal="current_avg_age_days",
        volume_signal="open_count",
        instance_signal="open_count",
        manual_step="manually chasing ageing service processes to keep the queue moving",
        unit="days",
        case_noun="ageing service processes in the queue",
        residual="processes blocked on a decision a person must make",
    ),
    # ---- STRS benefits ---------------------------------------------------
