"""
R17-A3 / T2 + T4 — Java application operational signal extraction.

Turns the operational artifacts produced by :mod:`discovery.ingest.java_app`
(Actuator metric samples + application log entries) into the operational SIGNAL
the story's Section 1 calls for: where a running Java application shows runtime
friction an agent could help address. Phase one reads the OPERATIONAL surface
only — what the running application reports about itself — and never the
application's source code (that is the separate 1.8 code-and-structure phase,
AC8).

The four operational signal types (R17-A3 §1)
---------------------------------------------
1. **Error patterns / error-rate** — error and exception volume over the logs and
   the Actuator-reported request error rate. See :func:`extract_error_signal`.
...
4. **Resource pressure** — JVM heap and CPU pressure reported by Actuator.
   See :func:`extract_metrics_signal`.
...
configuration files. That distinction is the operational/code boundary the story
draws between phase one (this story) and phase two (1.8).

Provenance (R16-B1 / T4, AC4)
-----------------------------
Every record produced by the ingestor carries a fully-populated, OBSERVED
:class:`~app.provenance.EvidencePointer` built by :func:`build_evidence_pointer`
(``source_system='java_app'``, ``origin='observed'``) — operational signals are
directly measured, so they are first-class observed evidence, never inferred
(R17-A3 §3, Architectural Note "Operational signals are observed evidence").

Downstream shape (T5 / AC5)
---------------------------
:func:`build_java_app_corroboration_payload` aggregates the records into the
block the corroboration engine reads for COR-09 (see
``app/corroboration_engine.py`` and ``discovery/packs/corroboration_rules.py``):
an ``operational_friction`` block ({fired, timestamp, services, reasons}) plus
the per-service rollup. A Java-app operational signal corroborating a finding in
another system (e.g. an error-rate rise corroborating a ServiceNow incident
spike for the same service) elevates confidence exactly as cross-system
corroboration does elsewhere.
"""
...
#: JVM heap used-ratio at or above which a sample shows memory pressure.
...
#: Log levels treated as errors for error-pattern counting.
...
def build_evidence_pointer(
    ...
    """Build the R16-B1 EvidencePointer for one Java-app operational signal (T4).
...
def extract_metrics_signal(metric_records: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    Reads the normalised Actuator samples (health, request metrics, JVM/CPU
    gauges) and derives the operational behaviour of the running application:
    peak error rate and p95 latency, latency degradation (rising p95 across the
    window), throughput decline, and resource pressure (heap/CPU). All values are
    counts/rates/gauges reported by the application about itself (AC8).
    """
    ...
    heaps = [v for v in (_as_float(m.get("jvm_memory_used_ratio")) for m in ordered) if v is not None]
    cpus = [v for v in (_as_float(m.get("system_cpu_usage")) for m in ordered) if v is not None]
...
def _friction_reasons(error: Dict[str, Any], metrics: Dict[str, Any], clusters: List[Dict[str, Any]]) -> List[str]:
    ...
    is the audit trail behind ``operational_friction.fired``.
    """
    reasons: List[str] = []
...
def build_service_signal(service: str, records: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    window so the corroboration engine can window it (R17-A3 §3).
    """
...
def build_java_app_signal(records: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    the corroboration engine (COR-09).
    """
...
#: keys COR-09 off this exact system id, so the block MUST be fed under this key.
...
def build_java_app_corroboration_payload(
    ...
    Wraps :func:`build_java_app_signal` under the ``'java_app'`` key that the
    corroboration engine's ``_find_corroboration_block('java_app', …)`` recognises.
    The engine reads the ``operational_friction`` block to fire COR-09 — a Java-app
    operational signal corroborating a finding in another connected system (e.g. an
    error-rate rise corroborating a ServiceNow incident spike for the same service)
    elevates confidence, because operational signals are first-class observed
    evidence (R17-A3 §3).
    ...
    """
    return {JAVA_APP_CORROBORATION_KEY: build_java_app_signal(records)}
