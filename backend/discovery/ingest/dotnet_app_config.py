...
The .NET counterpart to :mod:`discovery.ingest.java_app_config` (R17-A3 / T3),
deliberately parallel to it (R17-A4 §6, "A matched pair with Java"). Phase one of
the Java/.NET enterprise-application scope is **configured, not auto-discovered**:
AgentIQ does NOT scan a customer network to find .NET apps; each deployment
explicitly declares which applications are in scope. This module owns that
declaration and the safe resolution of the credentials needed to read them.

What a target declares
----------------------
Each :class:`DotNetAppTarget` names exactly one running .NET application AgentIQ
is allowed to read:

  * ``app_id``          — stable identity; the artifact-id prefix and per-app
                          checkpoint key (so a signal traces back to the right
                          service — R16-B1).
  * ``diagnostics_url`` — base URL of the .NET health/diagnostics surface
                          (ASP.NET Core health checks, runtime metrics /
                          EventCounters). Operational surface only (R17-A4 §1).
  * ``log_source``      — where the application's logs are read from (a file path
                          or a log endpoint reference).
  ...
  * ``metadata``        — non-secret service metadata (service name, owning team,
                          runtime) used to connect the signal back to the right
                          service for cross-system corroboration (R17-A4 §3).
  * ``credential_ref``  — a *reference* (a vault connector key), NOT a credential.
                          The actual secret — API key, token, username/password,
                          or certificate reference — is resolved at ingest time
                          from the credential vault, never stored here.
...
per-run credential context (a DB-sourced vault token isolated per org/run — the
same mechanism the SaaS connectors use, see
...
secret-looking field (``password``/``token``/``api_key``/``connection_string``/
``certificate``/...). A misconfigured deployment that pastes a credential into
config surfaces as a rejected target rather than silently persisting a plaintext
secret.
...
#: through into config files or logs. Note the *_ref forms (``credential_ref``,
...
class DotNetAppTarget:
    """One configured .NET application AgentIQ is allowed to read (R17-A4 §2).

    A target is pure, non-secret configuration. The credential needed to read the
    application's diagnostics endpoint / logs is referenced by ``credential_ref``
    and resolved from the vault at ingest time — it is never stored on the target.
    """
    ...
    #: Vault connector key naming where this app's secret lives (token / api key /
    #: username+password / certificate reference). None means the endpoint needs
    #: no credential (e.g. an internal unauthenticated health endpoint).
...
def _raw_target_entries(org_id: str) -> List[Dict[str, Any]]:
    ...
    network discovery is ever performed (R17-A4 §2).
    """
    if not is_live():
...
def resolve_secret(
    ...
    *,
    ...
    Resolution mirrors the SaaS connectors and the Java ingestor: the per-run
    credential context first (a DB-sourced vault token/secret, isolated per
    org/run via ``contextvars``), then an env var as a CLI/standalone fallback.
    The resolved value may be an API key, token, or a certificate reference — the
    vault is the single place it lives. Returns ``None`` when the target needs no
    credential (``credential_ref`` is None) or none is configured.
    ...
    # Env fallback key: DOTNET_APP_TOKEN for the default ref, else {REF}_TOKEN.
    env_key = (
        "DOTNET_APP_TOKEN"
