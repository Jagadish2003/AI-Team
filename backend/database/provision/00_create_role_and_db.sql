...
--
-- It is NOT required if the role + database already exist (e.g. on your local
-- machine, or when your DBA / managed-Postgres console / Terraform created them
-- by other means). The app and provision scripts connect to an existing
-- database; only the role + database must pre-exist. This file is the packaged
-- shortcut for that one-time step.
--
