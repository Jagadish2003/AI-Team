...
## How issuing works (AWS signing)

The private Ed25519 signing key lives in **AWS Secrets Manager** and is used by
the DevOps Lambda behind `LICENSE_API_URL`; it never leaves AWS. Issuance POSTs
the payload to that service, which signs it and returns the license token. The
shipped app verifies offline against the matching public key, baked into
`CLOUDFULCRUM_PUBLIC_KEY` in `app/licensing.py`.

```
license_ops.py issue  ──► POST payload ──► LICENSE_API_URL (AWS signs w/ Secrets Manager key)
                      ◄── { license_token }
                      ──► record in license_registry + issuance_audit
CLOUDFULCRUM_PUBLIC_KEY (app/licensing.py, shipped)  ──► offline verification
```

### Keypair provisioning (DevOps)

The keypair is provisioned and held by DevOps in AWS Secrets Manager
(`agentiq-licenseing-private-key`); the public half is baked into
`CLOUDFULCRUM_PUBLIC_KEY`. There is no local key-generation script in the repo —
signing is a service call. (A local `--signer local` path still exists for
isolated testing, but you must supply your own key via `--private-key`; no key
ships in `backend/license/`.)
...
|------|----------------|----------------|
| **Private signing key** | AWS Secrets Manager (`agentiq-licenseing-private-key`), used only by the `LICENSE_API_URL` Lambda; never in the repo or env | _<fill in: team + secrets-manager path>_ |
