"""
SQL Server operational signal ingestor — T2-S11-A.

Runs three signal queries against the declared SQL Server scope and returns
structured signals consumed by the sqlserver_opsignal detector set.

Called by the runner when 'sqlserver' is in the selected systems list.
All data access goes through execute_query() — no direct connection opened.

Windows auth vs SQL auth:
    SQL auth:     username_key resolves to a non-empty string → UID/PWD used in driver.
    Windows auth: username_key resolves to an empty string  → Trusted_Connection=yes
                  used in the driver layer; ingestor is unaffected.
"""
...
# (ServiceNow on-prem, Jira on-prem, generic service management databases)
# ---------------------------------------------------------------------------
...
TICKET_VOLUME_QUERY = """
SELECT CAST([{date_col}] AS DATE) AS [ticket_date],
       COUNT(*) AS [ticket_count]
FROM [{schema}].[{table}]
WHERE [{date_col}] >= DATEADD(day, -90, GETDATE())
...
SELECT [{priority_col}] AS [priority],
       ...
       AVG(DATEDIFF(hour, [{date_col}], GETDATE())) AS [avg_age_hours]
FROM [{schema}].[{table}]
...
def _process_ticket_volume(result: Any) -> dict:
    ...
    if date_idx is None or count_idx is None:
        logger.warning(
            "sqlserver_ingestor: ticket_volume — expected columns not found "
...
def _process_sla_breach(result: Any) -> dict:
    ...
    if total_idx is None or breached_idx is None:
        logger.warning(
            "sqlserver_ingestor: sla_breach — expected columns not found "
            "(columns=%s); setting degraded_signal=True",
            result.columns,
        )
        return {
...
def _process_queue_depth(result: Any) -> dict:
    ...
    if priority_idx is None or count_idx is None:
        logger.warning(
            "sqlserver_ingestor: queue_depth — expected columns not found "
            "(columns=%s); setting degraded_signal=True",
            result.columns,
        )
        return {
...
def ingest(
    ...
    """SQL Server operational signal ingestor.

    Called by the runner for every discovery run where sqlserver is in the
    selected systems list. Returns a structured dict consumed by the
    sqlserver_opsignal detector set.
    ...
    if scope is None:
        try:
            ...
            scope = ScopeDeclaration(
                ...
                schemas=["dbo"],
                tables=["dbo.ServiceTickets"],
                declared_at=datetime.now(timezone.utc),
                declared_by="system",
            )

    schema_name = scope.schemas[0] if scope.schemas else "dbo"
    ...
    try:
        ...
        if getattr(exc, "error_code", None) == "query_timeout":
            logger.warning(
                "sqlserver_ingestor: ticket_volume query timed out — degraded_signal=True"
            )
        else:
            logger.warning("sqlserver_ingestor: ticket_volume query failed: %s", exc)
        ticket_volume = _degraded_ticket_volume()
        degraded_count += 1
    except Exception as exc:
        logger.warning("sqlserver_ingestor: ticket_volume unexpected error: %s", exc)
        ticket_volume = _degraded_ticket_volume()
        degraded_count += 1
    ...
    try:
        ...
        if getattr(exc, "error_code", None) == "query_timeout":
            logger.warning(
                "sqlserver_ingestor: sla_breach query timed out — degraded_signal=True"
            )
        else:
            logger.warning("sqlserver_ingestor: sla_breach query failed: %s", exc)
        sla_breach = _degraded_sla_breach()
        degraded_count += 1
    except Exception as exc:
        logger.warning("sqlserver_ingestor: sla_breach unexpected error: %s", exc)
        sla_breach = _degraded_sla_breach()
        degraded_count += 1
    ...
    try:
        ...
        if getattr(exc, "error_code", None) == "query_timeout":
            logger.warning(
                "sqlserver_ingestor: queue_depth query timed out — degraded_signal=True"
            )
        else:
            logger.warning("sqlserver_ingestor: queue_depth query failed: %s", exc)
        queue_depth = _degraded_queue_depth()
        degraded_count += 1
    except Exception as exc:
        logger.warning("sqlserver_ingestor: queue_depth unexpected error: %s", exc)
        queue_depth = _degraded_queue_depth()
        degraded_count += 1
