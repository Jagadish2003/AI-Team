"""
PostgreSQL operational signal ingestor — T2-S12-A.
...
Driver: psycopg2-binary. All queries use "" double-quote identifiers.
PostgreSQL date arithmetic: NOW() - INTERVAL '90 days'.
SLA boolean: native boolean (= TRUE) with integer fallback (= 1) on type error.
"""
...
# PostgreSQL date arithmetic (NOW() - INTERVAL), native boolean support
# ---------------------------------------------------------------------------

TICKET_VOLUME_QUERY = """
SELECT "{date_col}"::date AS ticket_date, COUNT(*) AS ticket_count
FROM "{schema}"."{table}"
WHERE "{date_col}" >= NOW() - INTERVAL '90 days'
GROUP BY "{date_col}"::date ORDER BY ticket_date DESC
"""
...
def ingest(
    ...
    Uses psycopg2-binary. All queries use "" quoted identifiers.
    SLA breach query uses native boolean (= TRUE) with integer fallback (= 1)
    on type error — logs a warning when fallback is triggered.
    ...
    # Try native boolean first; fall back to integer (= 1) on type error.
    ...
    try:
        ...
        if getattr(exc, "error_code", None) == "query_timeout":
            ...
            # Possibly a type error on boolean column — retry with integer fallback
            logger.warning(
                "postgresql_ingestor: sla_breach boolean query failed (%s) — "
                "retrying with integer fallback",
                exc,
            )
            try:
                q2_fb = SLA_BREACH_QUERY_INT_FALLBACK.format(
                    sla_col="sla_breached", date_col="created_date",
                    schema=schema_name, table=table_name
                ).strip()
                result2_fb = execute_query(
                    config, scope=scope, query=q2_fb, org_id=org_id, run_id=run_id
                )
                query_count += 1
