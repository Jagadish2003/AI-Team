"""R17-D1 T3 - In-boundary resilience & graceful-failure contract tests.

The in-boundary provider must follow the same resilience posture as the hosted
reference implementation (R16-D2) so customers get consistent behaviour
regardless of provider mode (Story AC4).  This suite covers:
  ...
  T3-F  embed() degrades safely to [] on every failure path and shares the same
        retry posture (so embeddings never leave the boundary on a transient
        blip, and never raise).

Each test is labelled with the behaviour it guards so a failure maps directly
to the resilience guarantee that broke.
"""
...
def _configured(monkeypatch):
    """Configure a fully-specified in-boundary endpoint for every test."""
...
# ===========================================================================
# T3-B — retry is bounded by _MAX_RETRIES
# ===========================================================================
...
def test_t3e_retry_exhaustion_returns_graceful_failure(monkeypatch, provider):
    """Exhausting retries returns ok=False / text=None / provider='in_boundary'."""
    monkeypatch.setattr("time.sleep", lambda *_: None)
    monkeypatch.setattr(
...
def test_t3e_missing_config_returns_graceful_failure_without_network(monkeypatch, provider):
...
def test_t3f_embed_unexpected_error_does_not_raise(monkeypatch, provider):
    ...
    monkeypatch.setattr("time.sleep", lambda *_: None)
    monkeypatch.setattr(
        "urllib.request.urlopen", _always_raise(RuntimeError("boom"))
    )
