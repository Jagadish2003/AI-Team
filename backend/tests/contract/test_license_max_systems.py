...
# ===========================================================================
def test_get_max_systems_none_without_payload(monkeypatch):
    """No verifiable payload (no_license/invalid) → unlimited (None), not a block."""
    monkeypatch.setattr(
        LIMITS_STATUS, lambda *a, **k: {"status": "readonly", "reason": "no_license"}
    )
