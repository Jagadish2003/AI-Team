...
class TestEnrichmentEndpointRelationships:
    ...
    def _seed_run(self, run_id: str, opp_id: str) -> None:
        from app import db
        db.run_set(run_id, {"id": run_id, "runId": run_id, "status": "complete", "orgId": "default"})
    ...
    def test_observed_in_default_response(self, client, monkeypatch):
        ...
        opp = "opp-1"
        self._seed_run(run, opp)
        _seed_observed("default", run)
        _seed_inferred("default", run)

        r = client.get(f"/api/runs/{run}/opportunities/{opp}/enrichment", headers=_auth())
    ...
    def test_relationships_field_always_present(self, client, monkeypatch):
        ...
        opp = "opp-1"
        self._seed_run(run, opp)  # no edges seeded
