import json
import os
import time

import jwt
import requests
from dotenv import load_dotenv

load_dotenv()

LOGIN_URL = "https://login.salesforce.com"
TOKEN_URL = LOGIN_URL + "/services/oauth2/token"

CLIENT_ID        = os.environ.get("LS_CLIENT_ID")
USERNAME         = os.environ.get("LS_USER")
PRIVATE_KEY_PATH = "token_generation/life_sciences/server_ls.key"
TOKEN_FILE       = "token_generation/life_sciences/ls_token.json"

if not os.path.exists(PRIVATE_KEY_PATH):
    PRIVATE_KEY = ""
else:
    with open(PRIVATE_KEY_PATH, "r") as f:
        PRIVATE_KEY = f.read()


def save_token(access_token, instance_url):
    data = {
        "access_token": access_token,
        "instance_url": instance_url,
        "timestamp":    time.time(),
    }
    with open(TOKEN_FILE, "w") as f:
        json.dump(data, f)


def load_token():
    if not os.path.exists(TOKEN_FILE):
        return None
    with open(TOKEN_FILE, "r") as f:
        return json.load(f)


def get_new_token():
    if not PRIVATE_KEY:
        raise Exception("Cannot generate token: Private key is missing.")

    payload = {
        "iss": CLIENT_ID,
        "sub": USERNAME,
        "aud": LOGIN_URL,
        "exp": int(time.time()) + 300,
    }

    encoded_jwt = jwt.encode(payload, PRIVATE_KEY, algorithm="RS256")

    response = requests.post(
        TOKEN_URL,
        data={
            "grant_type": "urn:ietf:params:oauth:grant-type:jwt-bearer",
            "assertion":  encoded_jwt,
        },
    )

    if response.status_code != 200:
        raise Exception(f"Token generation failed: {response.text}")

    data         = response.json()
    access_token = data["access_token"]
    instance_url = data["instance_url"]

    save_token(access_token, instance_url)

    return access_token, instance_url


def get_token(force_refresh=False):
    token_data = load_token()

    if not token_data or force_refresh:
        return get_new_token()

    if time.time() - token_data["timestamp"] > 7000:
        return get_new_token()

    return token_data["access_token"], token_data["instance_url"]


def make_request(url, access_token):
    headers  = {"Authorization": f"Bearer {access_token}"}
    response = requests.get(url, headers=headers)

    if response.status_code == 401:
        print("Token expired. Refreshing...")
        access_token, instance_url = get_new_token()
        headers["Authorization"]   = f"Bearer {access_token}"
        response                   = requests.get(url, headers=headers)

        return response, access_token, instance_url

    return response, access_token, None


def main(force_refresh=False):
    access_token, instance_url = get_token(force_refresh=force_refresh)
    return access_token, instance_url


if __name__ == "__main__":
    main()