...
export default function SourceIntakePage() {
  ...
  const { uploadedFiles, sampleWorkspaceEnabled, addMockFile, removeFile, setSampleWorkspaceEnabled } = useSourceIntakeContext();
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 pb-28">
        <div className="mb-6">
          ...
          <div className="mt-1 text-sm text-muted">
            <span className="text-text font-semibold">{connected.length}</span> sources already connected via Integration Hub. Add files for additional coverage.
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <UploadPanel
            ...
            onAddMock={() => { addMockFile(); push('Added a mock upload file.'); }}
            onRemove={(id) => { removeFile(id); push('Removed file.'); }}
          ...
          <SampleWorkspacePanel
            ...
            onEnable={() => { setSampleWorkspaceEnabled(true); push('Sample workspace enabled.'); }}
            onLearnMore={() => push('More details available in Sprint 2.')}
          />
        </div>

        <div className="mt-4">
          <ReadySourcesSummary connectedNames={connectedNames} fileCount={uploadedFiles.length} sampleEnabled={sampleWorkspaceEnabled} />
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-bg/90 backdrop-blur">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-4 py-3">
            <Button variant="secondary" onClick={() => nav('/integration-hub')}>Back</Button>
            <Button
              variant="primary"
              disabled={!canBegin}
              title={!canBegin ? 'Connect at least one source to continue' : undefined}
              onClick={() => push('Discovery run started (mock).')}
            >
              Begin Discovery
            </Button>
          </div>
