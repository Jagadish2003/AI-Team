...
export default function ConnectorTile({
  connector, icon, selected, onSelect, onPrimary
}: { connector: Connector; icon: String, selected: boolean; onSelect: ()=>void; onPrimary: ()=>void }) {
  ...
  return (
    <div onClick={onSelect} className={`h-[140px] cursor-pointer rounded-xl border ${selected ? 'border-accent/60 bg-panel2' : 'border-border bg-panel'} p-3 hover:bg-panel2`}>
      <div className="flex items-start justify-between gap-2">
        {/* Added Icons for standard connectors */}
  ...
  className="w-full"
  onClick={(e: React.MouseEvent) => { e.stopPropagation(); onPrimary(); }}
