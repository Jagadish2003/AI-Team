...
2. **Validation** — manifest schema + the author's fixtures + lint, through the
   *same* `check_pack_directory` the author ran locally, **bounded** by the
   sandbox (2.0-C3 T6 / AT-841; see `pack_sandbox_validation.md`). One code path
   means "it passed on my machine" and "it passed on install" cannot diverge.
...
Installation records a pack; activation is a separate decision, and **everything
that can move underneath an installed pack is re-checked at activation** rather
than trusted from install time: the sandbox validation (T6 — the manifest and the
author's stored fixtures, re-judged against today's platform), the platform
version, and the org's certification floor. A pack that was installable last month is not automatically
...
`installed_packs` (migrations 0036 and 0037, mirrored into `provision.sql`): one
row per `(org, pack)`, re-install is an upgrade that bumps `revision`. `fixtures`
and `validation` (0037) hold the author's cases and the most recent sandbox
verdict — activation re-runs the former, and the latter is what makes "why will
this not activate" answerable without re-uploading the bundle.
...
| `GET /api/packs/installed/{packId}/validation` | analyst+ | the sandbox verdict; the reasons quote partner text |
...
gates, the registry, and the API. Sandbox validation (AT-841) layered onto gate 2
and added the activation-time re-run — see
[`pack_sandbox_validation.md`](pack_sandbox_validation.md). Wiring an active
installed pack into the discovery runner's execution path is a separate
integration that reads `installed_pack_config`.
