...
| **Opportunities API + Executive report** (`projection_copy_guard.py`) | `scrub_opportunity_narratives` runs on **both serve paths** in `main.py`. Both live routes compose their responses from stored opps *directly* rather than through `build_executive_report`, so a guard living only in the report engine would miss them — this was caught by CI, not by inspection. `scrub_executive_summary` guards the summary paragraph at the report boundary. Scrubbing returns **copies**: the stored opportunity is never rewritten, because a replay must serve what the run produced, and silently editing history to satisfy a guard would be its own kind of dishonesty. |
